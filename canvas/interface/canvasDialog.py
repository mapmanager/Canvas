...
class aboutDialog(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        self._versionDict = canvas.canvasUtil.getVersionInfo()
        
        self.buildUI()

    def buildUI(self):
        self.setWindowTitle('About Canvas')

        vBoxLayout = QtWidgets.QVBoxLayout()
        
        _textWidget = QtWidgets.QTextEdit()
                
        for k,v in self._versionDict.items():
            oneLine = f'{k} : {v}'
            _textWidget.append(oneLine)

        vBoxLayout.addWidget(_textWidget)
